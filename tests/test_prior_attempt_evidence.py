...
def test_the_carried_dropped_count_reaches_the_prompt():
    """`failing_tests_dropped` carried by `_record_prior_attempt_evidence`
    is what the coder must read as the remainder — not the word cap's own
    count of ids it could not fit (96,265 dropped used to render as
    "… and 65 more")."""
    evidence = {
        "from_attempt": 1,
        "findings": [],
        "failing_tests": [f"tests/test_x.py::test_{i}" for i in range(3)],
        "failing_tests_dropped": 800,
    }
    block = build_prior_attempt_evidence(evidence)
    assert "… and 800 more failing test id(s)" in block, block
