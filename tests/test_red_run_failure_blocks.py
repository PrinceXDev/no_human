...
async def test_persisted_failure_blocks_are_bounded_not_the_full_unbounded_list(
    ...
    # Ground truth is the FIXTURE, not the code under test: exactly 20
    # distinct `uniqueLongFailure<i>` signatures went in, so the number the
    # event text names IS the kept count and 20 minus it IS the true drop.
    # Deliberately no call into `bound_failure_blocks` here — an oracle that
    # is the walk under test would still agree with a renderer that kept one
    # block more than it persisted, or with a drop count one short.
    outcome, attempts, events, task = await _run_attempt_with_result(
        store, tmp_path, bare_repo, tr)
    ...
    assert test_events, events
    text = test_events[0]["text"]
    rendered = re.findall(r"uniqueLongFailure(\d+) certain", text)
    assert 0 < len(rendered) < 20, (len(rendered), text[-400:])
    expected_dropped = 20 - len(rendered)
    assert f"... {expected_dropped} more failing blocks" in text, text[-400:]

    persisted = _persisted(attempts[-1])
    signatures = []
    for block in persisted["failure_blocks"]:
        m = re.search(r"uniqueLongFailure(\d+)", block)
        assert m, block[:200]
        signatures.append(m.group(1))
    assert signatures == rendered, (signatures, rendered)
    assert persisted["failure_blocks_dropped"] == expected_dropped, persisted
    assert sum(len(b) for b in persisted["failure_blocks"]) <= runner.FAILURE_REPORT_MAX_CHARS
    ...
    # tests` is exactly `PrEvidence.tests`), so the `failure_blocks`
    # contribution to it inherits the bound. `failing_tests` is empty in this
    # fixture: that field is a separate, still-unbounded list (one id per
    # failing test — pre-existing, outside this change) and is NOT what the
    # size check below measures.
...
# --------------------------------------------------------------------------- #
# The two plain-branch exits nothing above drives: the pre-existing excuse   #
# and the "this change broke the runner" base-tree verdict. Both spread the  #
# same `base_test_results`; these pin that the spread keeps the blocks.      #
# --------------------------------------------------------------------------- #


async def test_the_pre_existing_excuse_write_keeps_its_failure_blocks(
    bare_repo, tmp_path, store,
):
    """`newly_failing == []` (every failing id already red on the base tree)
    with no owned id: the plain branch excuses the run and writes
    `test_results` with `pre_existing_failures`. That write must carry the
    blocks too — an excused red run is exactly the one a human reads later.
    """
    tap = _five_failing_tap()
    tr = runner.TestRunResult(
        ran=True, ok=False, passed=55, failed=5, errors=0,
        command="node --test", output=tap[-8000:], full_output=tap,
        failure_blocks=runner._tap_failure_blocks(tap),
        failing_tests=["desktop/widget.test.mjs"],
    )

    async def every_id_is_red_on_base(self, repo, test_cmd, base, failing_tests,
                                      cwd=None, env_dependent=False):
        return []

    with patch.object(Orchestrator, "_newly_failing_vs_base", every_id_is_red_on_base):
        outcome, attempts, events, task = await _run_attempt_with_result(
            store, tmp_path, bare_repo, tr, owned=[])

    rows = [a for a in attempts if "pre_existing_failures" in _persisted(a)]
    assert rows, [_persisted(a) for a in attempts]
    persisted = _persisted(rows[-1])
    assert persisted["pre_existing_failures"] == ["desktop/widget.test.mjs"], persisted
    blocks = persisted.get("failure_blocks")
    assert blocks and len(blocks) == 5, persisted
    assert all(f"uniqueFailureSignature{i}" in b for i, b in enumerate(blocks, 1)), blocks


async def test_the_runner_broken_by_this_change_exit_keeps_its_failure_blocks(
    bare_repo, tmp_path, store,
):
    """An invocation error that does NOT reproduce on the base tree fails the
    attempt from the plain branch's `reproduces_on_base: False` write — the
    last exit that spreads `base_test_results`. Pinned so a rebuild-from-
    scratch there drops the blocks loudly instead of silently.
    """
    tap = _invocation_error_tap()
    tr = runner.TestRunResult(
        ran=True, ok=False, passed=9, failed=1, errors=0,
    ...
    async def runs_fine_on_base(self, repo, test_cmd, base, cwd=None,
                                env_dependent=False):
        return False

    with patch.object(Orchestrator, "_invocation_error_reproduces_on_base",
                      runs_fine_on_base):
        outcome, attempts, events, task = await _run_attempt_with_result(
            store, tmp_path, bare_repo, tr, owned=[])
    ...
    failed_rows = [a for a in attempts if a.get("status") == "failed"]
    assert failed_rows, attempts
    persisted = _persisted(failed_rows[-1])
    assert persisted.get("reproduces_on_base") is False, persisted
    blocks = persisted.get("failure_blocks")
    assert blocks and any("ERR_MODULE_NOT_FOUND" in b for b in blocks), persisted
